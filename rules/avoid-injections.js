module.exports = {
  ...
  create(context) {
    ...
    return {
      [`CallExpression[callee.property.name=${rawStatements}][arguments.0.type!='Literal']`](node) {
        check(context, node);
      },
    };
  },
...
function check(context, node) {
  ...
  if (queryNode.type === 'TemplateLiteral' && queryNode.expressions.length === 0) {
    return;
  }
  ...
  if (queryNode.type === 'Identifier') {
    ...
    if (
      ...
      (queryVariableDefinition.init.type === 'TemplateLiteral' && queryVariableDefinition.init.expressions.length === 0)) {
      return;
    }
  }
